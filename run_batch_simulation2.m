...
%% 0. SET TEST MODE
% =========================================================================
is_test = false; % Set to false for a full run
%% 1. DEFINE MODEL LIBRARIES
creation_relvar_values = [0.01, 0.1, 0.2, 0.5, 1, 2, 5, 10];
creation_meantime_values = [0.1, 0.2, 0.5, 1, 2, 5, 10];
degradation_relvar_values = [0, 0.5, 1, 2, 5, 10];

degradation_meantime_values = 1;
[creation_models, degradation_models] = define_models_("Models", ...
    creation_relvar_values = creation_relvar_values, ...
    creation_meantime_values = creation_meantime_values, ...
    degradation_relvar_values = degradation_relvar_values, ...
    degradation_meantime_values = degradation_meantime_values);
%% 2. SET SIMULATION PARAMETERS
simulation_parameters = define_parameters_(creation_models, degradation_models,...
    numTrajectories = 1e4, ...
    T0_over_meantime_c = [-10, -10], ...
    dt_over_meantime_d = 1e-2, ...
    Tmax_over_meantime_d = 250, ...
    ss_time_over_meantime_d = 50);
% Override parameters for test mode
if is_test
    simulation_parameters.numTrajectories = 100;
end
% set directory to save data
[results_dir, stt_time] = set_directory_(is_test = is_test);
...
function [creation_models, degradation_models] = define_models_(ver, options)
    ...
    creation_relvar_values = options.creation_relvar_values;
    creation_meantime_values = options.creation_meantime_values;
    degradation_relvar_values = options.degradation_relvar_values;
    degradation_meantime_values = options.degradation_meantime_values;

    creation_models = {};
    degradation_models = {};
    [relvarc, meantc] = ndgrid(creation_relvar_values, creation_meantime_values);
    [relvard, meantd] = ndgrid(degradation_relvar_values, degradation_meantime_values);
...
function simulation_parameters = define_parameters_(creation_models, degradation_models, options)
    arguments
        ...
        options.dt_over_meantime_d = 0.1;
        options.Tmax_over_meantime_d = 110;
...
function [results_dir, stt_time] = set_directory_(options)
    arguments
    ...
    if options.is_test
        results_dir = sprintf("TESTRUN_%s", datetime("now", Format = "yyyyMMddHHmmss"));
    else
